...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const studyContext = [
      ...
      `Premier message utilisateur pour nommer le chat: ${
        typeof context?.firstUserMessage === 'string' ? context.firstUserMessage.slice(0, 500) : ''
      }`,
...
Si le contexte indique "Titre de chat requis: oui", utilise aussi l'outil set_chat_title une seule fois avec un titre court, utile, naturel et sans guillemets. Le titre doit faire 2 a 5 mots et etre genere uniquement a partir du champ "Premier message utilisateur pour nommer le chat", pas a partir des messages suivants ni de toute la conversation. Exemples: "Revision en biologie" ou "Photo de chien".
