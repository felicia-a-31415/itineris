...
const MIN_MAX_OUTPUT_TOKENS = 800;
const MAX_MAX_OUTPUT_TOKENS = 6000;
...
function getMaxOutputTokens() {
  const rawValue = Deno.env.get('OPENAI_MAX_OUTPUT_TOKENS');
  const parsedValue = rawValue ? Number.parseInt(rawValue, 10) : DEFAULT_MAX_OUTPUT_TOKENS;
  const safeValue = Number.isFinite(parsedValue) ? parsedValue : DEFAULT_MAX_OUTPUT_TOKENS;
  return Math.min(MAX_MAX_OUTPUT_TOKENS, Math.max(MIN_MAX_OUTPUT_TOKENS, safeValue));
}
